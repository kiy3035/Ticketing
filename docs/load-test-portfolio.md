...
| **stress+ PEAK 1500?** | 풀 30·batch 50·VT·캐시 동일, VU **1500**. k6 p95 **781 ms**, 에러 **0%**, **~808/s**, pending·active **30** 재등장, 대기열 **~2250**, max **~60 s**. §4.1, `queue-flow-stress-plus-1500vu-grafana-6panel.png`. |
| **stress+ batch 100?** | 풀 30 유지, `queue.batch-size`만 **100**. k6 p95 **2.79 s**, 에러 **0.14%**, `enter` 타임아웃, pending 피크 **~250**, **~590/s**. §4.1 대비 악화. §4.4, `queue-flow-pool30-batch100-1500vu-grafana-6panel.png`. |
...
**그래프**  
RPS는 **800–900/s** 부근에서 평탄해진다. VU는 1500인데 800 VU 캐시 런의 **~1000/s**와 비슷하거나 낮다. 유저 수만 늘리면 RPS가 비례한다는 가설은 이 데이터로는 성립하지 않으며, 처리 한도에 가깝다고 본다.

Hikari active **30**에 붙고 pending이 수십까지 오른다. 캐시 런에서 줄었던 DB 풀 큐잉이 부하 강화 뒤 다시 드러난다. `status` 캐시만으로 동시 접속 부하를 무한히 흡수할 수는 없고, 다른 경로·스케줄·풀 상한이 병목으로 다시 보인다.

대기열 인원은 **~2250**까지 거의 선형으로 증가한다. 입장 허용과 HTTP 처리가 유입 속도를 따라가지 못해 백로그가 쌓인다. 에러율은 0%에 가깝지만 체감 대기는 길어질 수 있다.

HTTP p95는 구간 대부분 낮다가 종료 직전에 크게 튄다. k6 max **~60 s**와 겹쳐 tail 지연과 스테이지 종료·램프다운이 겹친 구간으로 볼 수 있다. 해석할 때는 p95만이 아니라 max, pending, 큐 길이를 함께 본다.

JVM live threads **~30–31**은 VT 적용 시 기대 범위에 가깝다. OS 스레드 폭주보다는 DB 풀과 처리량 쪽 병목이 다시 부각된다.

k6 interrupted **889**는 피크 홀드가 짧고 이터레이션이 폴링 루프로 길어져 스테이지 끝에서 끊긴 시도가 많다는 뜻으로 본다. §5와 동일한 주의점이다.
...
§4.4 실측: stress+ 1500, 풀 30 고정에서 batch **50→100**이면 p95, pending, RPS, 타임아웃이 모두 악화된다. batch를 키우면 항상 좋아진다는 가설은 이 데이터로 기각된다.

기본값 50은 도메인에서 유도한 값이 아니라 `TicketingProperties`와 `activation-threshold` 50에 맞춘 튜닝에 가깝다. 적정값은 부하 테스트로 잡는다.
...
그래프와 k6: RPS **~550/s**, k6 **~430/s**로 §4.1의 **~800/s**보다 낮다. HTTP p95는 Grafana **~10–18 s**, k6 **9.86 s**로 §4.1 **~0.78 s**보다 크게 나쁘다. Hikari pending 피크 **~500**은 §4.1의 **~35**보다 한 단계 큰 큐잉이다. 대기열은 **1000+**로 유입 대비 소진이 더 느리다. `대기열 진입 201` 23건 실패, `http_req_failed` **0.05%**로 SLO 균열이 보인다.
...
|------|---------------|----------------|
| k6 p95 | **~781 ms** | **~2.79 s** |
| `http_req_failed` | **0%** | **0.14%** |
| 합성 RPS | **~808/s** | **~590/s** |
| Hikari pending 피크 | **~35** | **~250**, **~180** |
| 대기열 인원 | **~2250** | **~1750** |

active **30**은 유지되는데 pending이 두 자릿수에서 세 자릿수로 튄다. 풀 슬롯은 같고 연결 대기 쪽이 폭증한 형태다. HTTP p95는 **~5 s** 부근까지 올라가며 k6 max **~60 s**와 맞물린다. RPS는 §4.1보다 낮아 batch만 키웠다고 처리량이 늘지는 않았다.
