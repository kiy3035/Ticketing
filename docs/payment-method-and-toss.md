...
- **카드 결제**: 토스페이먼츠 **주문서형 위젯** 사용. 결제위젯 연동 키(test_gck_/test_gsk_)로 페이지 내 위젯 렌더 후 `widgets.requestPayment(orderId, ...)` 호출. successUrl 리다이렉트 후 서버에서 approve → complete. 카드 결제 기록·통계는 `payment` 테이블의 `payment_method`(CARD)로 구분.
