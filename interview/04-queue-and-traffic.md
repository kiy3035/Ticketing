...
> **🔴 Q6-2. `@Cacheable(TTL=2s)` 만 두면 안 되나요? 왜 `@CacheEvict` 까지 6곳에 박았나요?**
>
> **A.** TTL 2초만 두면 좌석이 잡힌 직후에도 잔여석이 최대 2초간 "있음"으로 노출됩니다. 사용자에게는 "보이는데 클릭하면 안 잡힘" 이 빈번해져 UX 손실이 큽니다. `@CacheEvict` 는 잔여석을 변화시키는 6개 도메인 이벤트 직후에 캐시를 즉시 무효화해 다음 polling 에서 정확한 값이 나오게 합니다.
>
> **6개 evict 지점**:
> 1. `HoldService:146` — 홀드 생성 직후 (잔여석 −1)
> 2. `HoldService:166` — 홀드 취소 (잔여석 +1)
> 3. `HoldCleanupScheduler:107` — 만료 홀드 정리 (잔여석 +1)
> 4. `ReservationConfirmedEventListener:41` — 예약 확정 (`AFTER_COMMIT`)
> 5. `ReservationService:208` — 예약 환불·취소 (잔여석 +1)
> 6. `SellerService:188` — 판매자 좌석 추가 (잔여석 +N)
>
> **TTL 과 evict 를 같이 두는 이유 — 이중 안전망**:
> - evict 로 즉시 반영 (UX 정확도)
> - 만약 evict 호출이 누락돼도 TTL 이 최악 2초 후 자동 복구 (안전망)
> - 한쪽만 두면 다른 쪽 단점이 그대로 노출됨 (TTL 단독은 최대 2초 stale, evict 단독은 누락 시 영구 stale 가능)
>
> **역할 분담**: 데이터 정합성(중복 예약 0%)은 **분산 락**이 잡고, 캐시는 **UX(체감 정확도)** 담당. 캐시가 잘못된 값을 보여줘도 실제 락 단계에서 차단되므로 정합성은 깨지지 않습니다.
