...
> **A.** 회전 + 탈취 탐지(family rotation)까지 가는 게 OAuth2 권고 스펙이지만, 일반 B2C 서비스에서는 단순 Refresh 저장 + 만료 시 재로그인이 더 흔합니다. 회전을 도입하면 탈취 탐지가 가능해지는 대신 정상 사용자도 일시 강제 로그아웃되는 트레이드오프가 있어서, 이 프로젝트 규모에는 단순 구조를 택했습니다.
