...
| **common** | 공통 응답, 글로벌 예외 처리, 캐시 키 | ApiResponse, GlobalExceptionHandler, CacheKeyConfig |
| **health** | DB/Redis/Kafka 헬스 체크 | DatabaseHealthIndicator, RedisHealthIndicator, KafkaHealthIndicator |
| **metrics** | 접속자 수, 대기열 메트릭 API | MetricsController, ActiveUserTracker, QueueMetrics |
| **scheduler** | 대기열 입장 허용, 대기열 정리, 홀드 만료 정리, 취소 공연 환불 | QueueProcessingScheduler, QueueCleanupScheduler, HoldCleanupScheduler, RefundForCancelledConcertScheduler |
...
- **예매 한 번에 타는 경로**  
  ...
  → DB 저장 후 **ApplicationEventPublisher.publishEvent(ReservationConfirmedEvent)**  
  → **ReservationConfirmedEventListener** (AFTER_COMMIT) → HoldStore.releaseHold(), SeatHoldEventPublisher.publish(RESERVATION_CONFIRMED)
